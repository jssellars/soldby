...
(function () {
  ...
  function onInit() {
	   ...
	   function applyUsFilter(product) {
		 ...
		 const name = product.dataset.sellerName || '';

		 if (name.includes('Amazon') || country === 'US') {
		   product.classList.remove('product--filtered');
		 } else {
	   ...
	   function createFilterToggle() {
		 ...
		 label.className = 'seller-filter-toggle';
		 label.style.cursor = 'pointer';

		 const checkbox = document.createElement('input');
		 checkbox.type = 'checkbox';
		 checkbox.id = 'seller-filter-checkbox';
		 checkbox.checked = isFilterEnabled();
		 checkbox.addEventListener('change', () => {
		   setFilterEnabled(checkbox.checked);
		   document
			 .querySelectorAll('[data-seller-name]')
			 .forEach(applyUsFilter);
		 });
